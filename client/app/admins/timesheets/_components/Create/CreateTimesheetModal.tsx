...
export function CreateTimesheetModal({
  ...
  const truckOptions = useMemo(() => {
    console.log("🚚 Building truckOptions from trucks:", trucks?.length || 0);
  ...
  const fetchEquipment = useCallback(async () => {
    try {
      ...
      if (equipmentData.length > 0) {
        ...
        const uniqueTags = [
          ...new Set(
            equipmentData.map((e: { equipmentTag?: string }) => e.equipmentTag)
          ),
        ];
  ...
  const fetchMaterialTypes = useCallback(async () => {
    try {
      ...
      console.log("📦 Processed Material Types:", materialTypesData.length);
      ...
      if (!equipment || equipment.length === 0) {
        ...
        apiRequest("/api/v1/admins/equipment", "GET")
          .then((data) => {
            ...
            console.log("✅ Equipment re-fetched:", equipmentData.length);
            setEquipment(equipmentData);
          })
          .catch((error) => {
