    ...
    const fetchEmployeesAndMaterials = async () => {
      try {
        const options = await apiRequest(
          "/api/v1/admins/report/tasco/filters",
          "GET"
        );
        const formattedEmployees =
          options.employees?.map((emp: { id: string; name: string }) => ({
            id: emp.id,
            name: emp.name,
          })) || [];
    ...
    <div className="w-full p-4 grid grid-rows-[3rem_2rem_1fr] gap-5">
      ...
      <div className="h-10 w-full flex flex-row justify-between gap-4">
        <div className="flex flex-row gap-4">
          ...
          >
            <SelectTrigger className="w-[350px] bg-white text-gray-900 font-medium [&>span]:text-gray-900">
