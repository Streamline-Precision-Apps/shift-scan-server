...
export default function TascoReport({
  ...
  const buildQueryString = () => {
    ...
    if (externalFilters?.equipmentId?.length) {
      params.append("equipmentIds", externalFilters.equipmentId.join(","));
    }
    if (externalFilters?.materialType?.length) {
      params.append("materialTypes", externalFilters.materialType.join(","));
    }
  ...
  const onExport = (
    ...
    const formatFLoads = (fLoadDetails: any[]) => {
      if (!Array.isArray(fLoadDetails) || fLoadDetails.length === 0) return "-";
      return fLoadDetails
        .map(
    ...
    if (exportFormat === "csv") {
      ...
      import("xlsx").then((XLSX) => {
        ...
        exportData.forEach((row) => {
          if (Array.isArray(row.fLoadDetails) && row.fLoadDetails.length > 0) {
